...
const search = (key) => {
    let response = GET(`https://www.duokan.com/store/v0/lib/query/onebox?start=0&count=10&s=${encodeURI(key)}&source=2%2C5%2C7`)
    let array = []
...
const detail = (url) => {
    ...
    let book = {
        ...
        update: $.updated,
        lastChapter: $.latest,
        catalog: `https://www.duokan.com/store/v0/fiction/detail/${$.fiction_id}?chapter_id=0&fid=${$.fiction_id}`
    }
...
const catalog = (url) => {
    ...
    $.item.toc.forEach((chapter, index) => {
        array.push({
            name: chapter.title,
            url: `https://www.duokan.com/drm/v0/fiction/link?chapter_id=${chapter.seq_id}&format=jsonp&withid=1&fiction_id=${url.query("fid")}`,
